...
static void ocuriparamfree(char** params);
...
ocurifree(OCURI* ocuri)
{
    ...
    if(ocuri->paramlist != NULL) {
        char** p = ocuri->paramlist;
        for(;*p;p+=2) {
	    free(*p);
	    if(p[1] != NULL) free(p[1]);
        }
        free(ocuri->paramlist);
    }
    free(ocuri);
}
...
ocuridecodeparams(OCURI* ocuri)
{
    ...
    if(ocuri->paramlist != NULL)
	ocuriparamfree(ocuri->paramlist);
    ...
    *cq = '\0';
    ...
    plist[nparams] = NULL;
    free(params);
    if(ocuri) ocuri->paramlist = plist;
    return 1;
}

static void
ocuriparamfree(char** params)
{
    char** p;
...
const char*
ocurilookup(OCURI* duri, const char* key)
{
    int i;
    /* make sure that duri->paramlist exists */
    if(duri == NULL || key == NULL) return NULL;
    if(duri->paramlist == NULL) {
	if(duri->params == NULL) return NULL;
	i = ocuridecodeparams(duri);
	if(i == 0) return NULL;
    }
    i = ocfind(duri->paramlist,key);
    if(i >= 0)
	return duri->paramlist[(2*i)+1];
    return NULL;
}
...
*/
int
ocuriparamdelete(char** params, const char* key)
{
    int i;
    char** p;
    char** q;
...
*/
char**
ocuriparaminsert(char** params, const char* key, const char* value)
{
    int i;
    char** newp;
    size_t len;
...
*/
int
ocuriparamreplace(char** params, const char* key, const char* value)
{
    int i;
    if(params == NULL || key == NULL) return 0;
    i = ocfind(params,key);
